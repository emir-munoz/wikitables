...
public class WikiTableAccess
{
	...
	// /**
	// * Get the DataProg, i.e., the patterns for the data.
	// *
	// * @param columnValues List of strings from which you wish to obtain patterns.
	// */
	// @SuppressWarnings("unchecked")
	// public void getDataProg(ArrayList<String> columnValues)
	// {
	...
	// dataProg.setTokens((ArrayList<String>) columnValues);
	//
	// ArrayList<String> listAux = (ArrayList<String>) columnValues.clone();
	...
	// // TODO: Just taking the first pattern for now
	// // while (listIter.hasNext())
	// if (listIter.hasNext())
	// _log.debug("Significant pattern: " + (String) listIter.next() + "\n");
	// // System.out.println("Significant pattern: " + (String) listIter.next() + "\n");
	// }
	// }

	// @SuppressWarnings("deprecation")
	// public List<String> getColumnValues(FeaturesExtractorYData extractor, int col)
	// {
	...
	// String cellText = StringUtils.trimToEmpty(eleChild.toText());
	// // cellText = cellText.replaceAll("[\t]+", "").replaceAll("[\n]+", "");
	// if (row != 0 && !StringUtils.escape(cellText).equals("\\u00A0"))
	// {
	// cellText = URLUTF8Encoder.encode(cellText.replace("–", "-")).replace("+", " ");
