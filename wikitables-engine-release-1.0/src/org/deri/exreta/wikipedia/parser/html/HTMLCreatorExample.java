...
public class HTMLCreatorExample {
	...
	public static String testWikipediaENAPI(String title) {
		return testWikipediaENAPI(title, "http://en.wikipedia.org/w/api.php",
				Locale.ENGLISH);
	}

	public static void testWikipediaText(String rawWikiText, String title,
			String apiLink, Locale locale) {
		String[] listOfTitleStrings = { title };
		String titleURL = Encoder.encodeTitleLocalUrl(title);
		User user = new User("", "", apiLink);
		String mainDirectory = "./temp/";
		...
		String imageDirectory = "./temp/WikiImages";
		...
		try {
			...
			APIWikiModel wikiModel = new APIWikiModel(user, db, locale,
					"${image}", "${title}", imageDirectory);
			DocumentCreator creator = new DocumentCreator(wikiModel, user,
					listOfTitleStrings);
			// create header and CSS information
			creator.setHeader(HTMLConstants.HTML_HEADER1
					+ HTMLConstants.CSS_MAIN_STYLE
					+ HTMLConstants.CSS_SCREEN_STYLE
					+ HTMLConstants.HTML_HEADER2);
			creator.setFooter(HTMLConstants.HTML_FOOTER);
			wikiModel.setUp();
			creator.renderToFile(rawWikiText, title, new HTMLConverter(),
	 ...
	 * >http://en.wikipedia.org/w/api.php</a> for the english Wikipedia) and
	 * write the generated HTML file to the<code>c:/temp/</code> Windows
	 * directory.
	 * 
	 * @param title
	 ...
	 * @return the redirected link title if a <code>#REDIRECT [[...]]</code>
	 *         link is set in the wiki text; <code>null</code> otherwise
	 */
	public static String testWikipediaENAPI(String title, String apiLink,
			Locale locale) {
		String[] listOfTitleStrings = { title };
		String titleURL = Encoder.encodeTitleLocalUrl(title);
		User user = new User("", "", apiLink);
	...
	public static void testCreator010() {
		testWikipediaENAPI("HTTP-Statuscode",
				"http://de.wikipedia.org/w/api.php", Locale.GERMAN);
	}

	public static void testCreator011() {
		testWikipediaENAPI("Main Page",
				"http://simple.wikipedia.org/w/api.php", Locale.ENGLISH);
	}
	...
	public static void testCreateText001() {
		testWikipediaText(TEXT_ISSUE_96,
